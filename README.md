...
2. Create a Blueprint from this repository. It creates the `routeboard` frontend and connects to `routeboard-api` over Render's private network.

Both Blueprints must be in the same Render workspace. The frontend calls the
Spring API only through server-side Next.js route handlers, so no browser CORS
